  ...
  return (
    <ChartContainer config={chartConfig} className="aspect-auto w-full" style={{ height: Math.max(rows.length * ROW_HEIGHT, MIN_CHART_HEIGHT) }}>
      <BarChart data={rows} layout="vertical" margin={{ left: 0, right: 40, top: 0, bottom: 0 }}>
        ...
        <YAxis dataKey="concernName" type="category" tickLine={false} axisLine={false} width={Y_AXIS_WIDTH} tick={<CategoryAxisTick />} />
        <Bar dataKey="count" fill="var(--color-count)" radius={4} barSize={BAR_SIZE} activeBar={false}>
